...
export class Auth {
  ...
  acceptInvite(token: string, payload: { password: string; first_name?: string; last_name?: string; phone?: string }): Observable<AuthenticatedSession> {
    const baseUrl = environment.apiUrl;
    return this.http.post<any>(`${baseUrl}/auth/accept-invite`, { token, ...payload }).pipe(
      map((res: ApiResponse<AuthenticatedSession>) => res.data),
