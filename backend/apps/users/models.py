...
class User(BaseDocument):
    ...
    HIDDEN_FIELDS = (
        "password_hash", "failed_login_attempts", "mfa_secret", "mfa_recovery_codes",
    )
...
class RolePermission(BaseDocument):
    """A per-organization override of a role's default permission set.

    Absence of a document for `(organization, role)` means "use
    `core.permissions.DEFAULT_ROLE_PERMISSIONS`" - see
    `core.permissions.effective_permissions`.  SUPER_ADMIN is never stored
    here; it always has every permission.
    """

    organization = ReferenceField("Organization", required=True)
    role = StringField(required=True, choices=[r for r in Role.ALL if r != Role.SUPER_ADMIN])
    permissions = ListField(StringField(), default=list)

    meta = {
        "collection": "role_permissions",
        "indexes": [{"fields": ("organization", "role"), "unique": True}],
    }

    def __repr__(self):
        return "<RolePermission {} ({})>".format(self.role, self.id)
