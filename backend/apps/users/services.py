...
from apps.users.models import EmailVerificationToken, PasswordResetToken, RefreshToken, User
from core.audit import record as audit_record
from core.constants import AuditAction, Role, UserStatus
...
def create_user(organization: Organization, data: dict, *, created_by: User = None) -> tuple:
    ...
    if created_by is not None:
        assert_can_assign_role(created_by.role, role)
...
def update_user(user: User, data: dict, *, editor: User = None) -> User:
    ...
    if "role" in data and editor and editor.role in (Role.SUPER_ADMIN, Role.ADMIN):
        ...
        new_role = validate_choice(data["role"], Role.ALL, "role")
        assert_can_assign_role(editor.role, new_role)
        user.role = new_role
