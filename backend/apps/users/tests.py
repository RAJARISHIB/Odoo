...
from apps.users.models import RefreshToken, RolePermission, User
from core.constants import Role, UserStatus
from core.security import generate_recovery_codes, generate_totp_secret, hash_password, totp_uri, verify_totp
...
class SecurityTestCase(ApiTestCase):
    ...
    def setUp(self):
        ...
        self.organization = Organization(
            name="Test Co", slug=self._unique("test-co"), code=self._unique("TC")[:4],
            email="org-{}@example.com".format(self._unique("")),
        ).save()

        self.employee = self._make_user(Role.EMPLOYEE, "employee")
        self.other_employee = self._make_user(Role.EMPLOYEE, "other")
        self.hr = self._make_user(Role.HR, "hr")
        self.admin = self._make_user(Role.ADMIN, "admin")
        self.super_admin = self._make_user(Role.SUPER_ADMIN, "super")

    def tearDown(self):
        RefreshToken.objects.filter(user__in=[
            self.employee, self.other_employee, self.hr, self.admin, self.super_admin,
        ]).delete()
        RolePermission.objects.filter(organization=self.organization).delete()
    ...
    def _make_user(self, role: str, tag: str, *, password: str = "Password123") -> User:
        user = User(
...
class PrivilegeEscalationTests(SecurityTestCase):
    def test_hr_cannot_create_a_super_admin(self):
        ...
        response = self.post_json(
            "/api/v1/users",
            {"email": "new-super@example.com", "name": "New Super", "role": Role.SUPER_ADMIN},
            **self.auth_header(token),
        )
        self.assertEqual(response.status_code, 403)
    ...
    def test_hr_can_create_an_employee(self):
        ...
        response = self.post_json(
            "/api/v1/users",
            {"email": "new-employee@example.com", "name": "New Employee", "role": Role.EMPLOYEE},
            **self.auth_header(token),
        )
    ...
    def test_admin_cannot_promote_another_admin_to_super_admin(self):
        ...
        second_admin = self._make_user(Role.ADMIN, "second_admin")
    ...
    def test_admin_cannot_change_their_own_role(self):
        token = self._access_token(self.admin)
        response = self.patch_json(
            "/api/v1/users/{}".format(self.admin.id), {"role": Role.EMPLOYEE}, **self.auth_header(token)
        )
...
class AuditLogAndPermissionTests(SecurityTestCase):
    ...
    def test_role_permissions_are_configurable_per_organization(self):
        # role.manage is MFA-required (see core.permissions.MFA_REQUIRED_PERMISSIONS),
        ...
        response = self.get_json(
            "/api/v1/admin/roles/{}/permissions".format(Role.EMPLOYEE), **self.auth_header(token)
        )
        self.assertEqual(response.status_code, 200)
        self.assertIn("leave.create", self.body(response)["data"]["permissions"])

        updated = self.put_json(
            "/api/v1/admin/roles/{}/permissions".format(Role.EMPLOYEE),
            {"permissions": ["leave.view_own"]},
            **self.auth_header(token),
        )
        self.assertEqual(updated.status_code, 200)
        self.assertEqual(
            RolePermission.objects.get(organization=self.organization, role=Role.EMPLOYEE).permissions,
            ["leave.view_own"],
        )
